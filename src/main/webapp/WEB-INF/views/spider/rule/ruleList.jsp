...
#querydiv {
	...
	<body>
		...
		<div id="querydiv">
			<form name="queryForm" id="queryForm" action="#">
				...
				<a
					...
					href="${ctx}/spider/rule/create">新增</a>
			</form>
		</div>
		...
		<table id="contentTable"
			...
			<tbody>
				...
				<c:forEach items="${rules.content}" var="rule">
					<tr>
						<td>
							<a href="${ctx}/spider/rule/update/${rule.id}">${rule.code}</a>
						</td>
						<td>
							${rule.name}
						</td>
						...
						<td>
							<a href="${ctx}/spider/rule/delete/${rule.id}">删除</a>
						</td>
					</tr>
				</c:forEach>
			</tbody>
		...
		<tags:pagination page="${rules}" paginationSize="5" />
